...
# self-update channel asset (static musl):
...
`cc-switch update` and `update --check` work against this repository's releases.
The unsigned legacy channel is used (no `latest.json` manifest), so every release
must include the platform asset the updater selects — on Linux x86_64 that is
`cc-switch-cli-linux-x64-musl.tar.gz` (auto/musl preference) — plus
`checksums.txt` with SHA-256 lines for each asset. GitHub's asset digest also
satisfies checksum verification, but keep `checksums.txt` for older clients.
